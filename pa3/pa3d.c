...
void InitRoad()
{
	...
	shm.west_nums = 0;
	shm.east_nums = 0;
	shm.west_lock = Seminit(0);
	shm.east_lock = Seminit(0);
	shm.mem_lock = Seminit(1);
	...
	// mph: speed of car
{
	...
	int p, np, i, j;			// positions

	c = Getpid();			// learn this car's ID
	
	Wait(shm.mem_lock);
	if (shm.car_nums && shm.cur_dir != from) {
		if (from == EAST) {
			shm.east_nums++;
			Signal(shm.mem_lock);
			Wait(shm.east_lock);
	...
	if (from == shm.cur_dir) {	
		if (from == WEST && (shm.east_nums || shm.atGate)) {
			shm.west_nums++;
			Signal(shm.mem_lock);
			Wait(shm.west_lock);
			Wait(shm.mem_lock);
		...
		else if (from == EAST && (shm.west_nums || shm.atGate)) {
			shm.east_nums++;
			Signal(shm.mem_lock);
			Wait(shm.east_lock);
			Wait(shm.mem_lock);
	...
	for (i = 1; i < NUMPOS; i++) {
		...
		if (i == 1) {
			Wait(shm.mem_lock);
			shm.atGate = 0;
			if (from == WEST && !shm.east_nums && shm.west_nums) {
				Signal(shm.west_lock);
			}
			else if (from == EAST && !shm.west_nums && shm.east_nums) {
				Signal(shm.east_lock);
			}
			Signal(shm.mem_lock);
		} else {
	...
	Wait(shm.mem_lock);
	shm.car_nums--;
	if (from == WEST && shm.east_nums && !shm.car_nums) {
		Signal(shm.east_lock);
	} 
	else if (from == EAST && shm.west_nums && !shm.car_nums) {
		Signal(shm.west_lock);
	} 
	Signal(shm.mem_lock);
}
