	...
	// v: initial value of semaphore
{
	...
	for (int i = 0; i < MAXPROCS - 1; i++) {
		semtab[s].blockList[i] = 0;
	}
	semtab[s].blockCount = 0;
	semtab[s].head = 0;
	semtab[s].tail = 0;
	...
	// s: semaphore ID
{
	...
	if (semtab[s].valid) {
		semtab[s].value--;
		if (semtab[s].value < 0) {
			semtab[s].blockList[semtab[s].tail] = GetCurProc();
			semtab[s].tail = (semtab[s].tail + 1) % (MAXPROCS - 1);
			semtab[s].blockCount++;
			//DPrintf("Process %d is going to be blocked\n", GetCurProc());
			Block();
		}
	}
}
	...
	// s: semaphore ID
{
	...
	if (semtab[s].valid) {
		semtab[s].value++;
		if (semtab[s].blockCount != 0) {
			Unblock(semtab[s].blockList[semtab[s].head]);
			//DPrintf("Process %d was unblocked by %d\n", semtab[s].blockList[semtab[s].head], GetCurProc());
